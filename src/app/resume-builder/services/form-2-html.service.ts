import {
  WorkExperience,
  ResumeFormData,
} from './../interfaces/resume-form-model';
...
export class Form2HtmlService {
  ...
  convertForm2HTML(
    ...
    const workExpHeaderDiv: HTMLDivElement = this.createSectionHeader(
      formData.workExpParentForm.header || ''
    );
    ...
    formData.workExpParentForm.workExps?.forEach((workExp) => {
      ...
      const jobResponsibilities: HTMLElement[] = this.createWorkResponsibilites(
        workExp.description || ''
      );
  ...
  private createWorkResponsibilites(description: String): HTMLElement[] {
    const responsibilitiesContainer: HTMLElement = new DOMParser().parseFromString(
      description?.toString() || '',
      'text/html'
    ).documentElement;
    const immediateNodes: HTMLCollection = responsibilitiesContainer.querySelector(
      'body'
    )?.children;
    ...
    Array.from(immediateNodes).forEach((immediateNode: Element) => {
      if (
        immediateNode.tagName.toLocaleLowerCase() === 'ul' ||
        immediateNode.tagName.toLowerCase() === 'ol'
      ) {
  ...
  private listTagToChunks(ulTag: HTMLElement) {
    ...
    Array.from(ulTag.children).forEach((li) => {
      Array.from(li.childNodes).forEach((node) => {
        if (node.nodeType === 3) {
          const wordsSpanArray: HTMLElement[] = this.textToSpanArray(
            node.textContent || ''
          );
  ...
  private textToSpanArray(str: String): HTMLSpanElement[] {
    let span: HTMLSpanElement, text: Text;
  ...
  private createJobLocation(workExp: WorkExperience): HTMLDivElement {
    ...
    const jobLocationDiv: HTMLDivElement = this.createElement(
      'div'
    ) as HTMLDivElement;
    const jobLocation = [company, city, state, country]
      .map((value) => value)
      .join(', ');
  ...
  private createJobHeader(workExp: WorkExperience): HTMLDivElement {
    ...
    const jobHeader: HTMLDivElement = this.createElement(
      'div'
    ) as HTMLDivElement;
  ...
  private createJobDuration(workExp: WorkExperience): HTMLDivElement {
    const durationDiv: HTMLDivElement = this.createElement(
      'div'
    ) as HTMLDivElement;
  ...
  private createJobTitle(workExp: WorkExperience): HTMLDivElement {
    const jobTitleDiv: HTMLDivElement = this.createElement(
      'div'
    ) as HTMLDivElement;
  ...
  private createSectionHeader(title: String): HTMLDivElement {
    const workExpHeaderDiv: HTMLDivElement = this.createElement(
      'div'
    ) as HTMLDivElement;
    workExpHeaderDiv.innerHTML = title?.toString() || '';
